...
校巴站点作为独立“站”标记随已开启线路显示；默认线路全部关闭时站点也不显示，只开启某条线路时显示该线路途经站点。线上站点本体始终使用原始站点坐标，明确有上/下行区分时只在旁边显示方向标记。选中站点后会显示站名、明确标注的上下行方向、该站所有途经线路、当前按时刻表估算的小车数量，以及每条途经线路的预计下一班到站时间；站点弹窗的 ETA 不受当前线路筛选限制。
